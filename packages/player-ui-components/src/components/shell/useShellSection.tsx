 ...
 *
 * The tile is `available` only when the player reports `shellAvailable`, which
 ...
 *
 * Shared by the Electron, LAN and cloud settings screens so the three cannot
 * drift on the gate.
 */
export function useShellSection(): { section: SettingsSection; dialog: React.ReactNode } {
    const available = useSelector((state: RootState) => state.shellAvailable);
